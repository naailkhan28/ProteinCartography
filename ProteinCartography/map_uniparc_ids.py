...
# if necessary, mock the `uniprot.mapping` method (used by `map_uniparcids_bioservices`)
...
__all__ = ["map_uniparcids_bioservices", "map_uniparcids_rest"]
...
def map_uniparcids_bioservices(
    ...
    with open(input_file) as f:
        ids = [line.rstrip() for line in f if line.startswith("UPI")]
...
def map_uniparcids_rest(input_file: str, output_file: str, query_dbs: list, return_full=False):
    """
    Takes an input .txt file of accessions and maps to UniProt accessions.
    ...
    # open the input file to extract ids
    with open(input_file) as f:
        input_lines = [line for line in f if line.startswith("UPI")]
...
def main():
    ...
    if service == UniProtServices.BIOSERVICES.value:
        map_uniparcids_bioservices(input_file, output_file, query_dbs)
    else:
        map_uniparcids_rest(input_file, output_file, query_dbs)

    #Remove all UniParc IDs from input file to avoid errors later:
    with open(input_file, "r") as infile:
        lines = [line for line in infile if not line.startswith("UPI")]
